package de.honoka.demo.spring.oauth2.auth.service
...
import de.honoka.demo.spring.oauth2.auth.mapper.UserMapper
import de.honoka.demo.spring.oauth2.auth.security.LoginIdCache
...
class UserService(private val userMapper: UserMapper) : ServiceImpl<UserMapper, User>() {
    ...
    @PostConstruct
    fun init() {
        userMapper.findByUsername("admin")?.let { return }
    ...
    fun newLoginId(params: JSONObject): String {
        ...
        val loginId = "${user.username}-${UUID.randomUUID()}"
        LoginIdCache.cache.put(loginId, null)
        return loginId
    }

    fun login(params: JSONObject): String {
        val loginId = newLoginId(params)
        ...
        var res = HttpUtil.createGet(url).run {
            ...
            header(LoginIdCache.HEADER, loginId)
